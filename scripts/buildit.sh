...
docker run -d --hostname=pacifica-vm --name=pacifica-vm -v /sys/fs/cgroup:/sys/fs/cgroup:ro --privileged travis_local/pacifica/pacifica-vm:latest
docker exec pacifica-vm ansible-playbook -e keep_vcs=${KEEP_VCS} /etc/ansible/pacifica-vm/setup-play.yml
docker exec pacifica-vm ansible-playbook /etc/ansible/drupal-vm-6.0.1/provisioning/playbook.yml
